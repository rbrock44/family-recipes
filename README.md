...
- [Recipe Data Bundle](#-recipe-data-bundle)
...
Use the "Add to list" icon on a recipe (search results, favorites, recently
visited, or the recipe's own page) to add it to a new or existing list, and
set how many batches to make. Each list auto-aggregates the ingredients for
every recipe in it, scaled by batches, into a single checkable shopping list.
Ingredients that appear in multiple recipes are combined into one line. Add
your own items under "Other Ingredients" for anything not covered by a
recipe. Lists can be renamed, deleted, or shared with a link (via the share
icon) that lets anyone import a read-only copy of the list.
...
The app reads recipes from a single combined `src/assets/recipes.json`,
generated by `scripts/combineRecipes.js` from every file in
`src/assets/recipes/`. This means the app makes one HTTP request for the full
recipe list instead of one request per recipe. The combined file is
gitignored and not committed, it's regenerated:

- locally, automatically before `npm start` if it's missing (via the
  `prestart` hook), or manually with `npm run recipes:combine` after
  adding/editing recipe files
- in CI, as a build step before every deploy, so the packaged site always
  ships with the latest recipes
...
The first run generates `src/assets/recipes.json` automatically (see
[Recipe Data Bundle](#-recipe-data-bundle)) since it isn't committed to git.
...
- Build
    ...
    - Action(s): Combines recipes into `src/assets/recipes.json`, builds the application, then kicks off the gh page action to deploy the build output
